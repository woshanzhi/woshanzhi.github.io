...
画图的时候可以参考自己需要投稿的期刊的作者指南的要求进行绘图。
...
P1是论文原图，P2是Nano Banana Pro生成的，这效果太棒了。具体流程是在Gork(提示词为"请给出这个框架图的提示词”)中使用参考的框架图生成绘图提示词，然后复制绘图提示词到Gemini中使用Banana生成。简单小改一下提示词，框架图的草图不就有了吗?大概流程是，你找一个现有有论文的框架图，让grok生成提示词，你基于自己的方法改一下提示词，然后banana生成框架图。

可以自己看论文找到可以用于自己论文的模型框架图可以保留下来，根据自己模型等内容写提示词，将图片丢给豆包让他在这个基础上进行修改，然后自己根据生成的图片，自己进行画图修改。
## 十一、我自己的使用建议
